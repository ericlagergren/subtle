...
func TestEncode(t *testing.T) {
	for _, p := range pairs {
		for i, tt := range encodingTests {
			...
			testEqual(t, "#%d: Encode(%q) = %q, want %q",
				i, p.decoded, got, tt.conv(p.encoded))
		}
	}
}
...
func TestDecodeCorrupt(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		if tc.offset == -1 {
			if err != nil {
				t.Errorf("Decoder wrongly detected corruption in %q", tc.input)
			}
			continue
		}
		if !errors.Is(err, ErrCorrupt) {
			t.Fatalf("unexpected error in %q: %v", tc.input, err)
		}
	}
}
...
func TestNewLineCharacters(t *testing.T) {
	...
	for _, e := range examples {
		_, err := StdEncoding.DecodeString(e)
		if !errors.Is(err, ErrCorrupt) {
...
`
	...
	dec := NewDecoder(StdEncoding,
		NewlineFilteringReader(strings.NewReader(encoded)))
...
func TestDecoderIssue7733(t *testing.T) {
	...
	// 11 % 4 == 3, so we reject it out of hand.
	if len(s) > 0 {
		t.Errorf("DecodeString = %q", s)
	}
}
...
func BenchmarkEncode(b *testing.B) {
	sizes := []int{2, 16, 32, 64, 8192, 16384}
